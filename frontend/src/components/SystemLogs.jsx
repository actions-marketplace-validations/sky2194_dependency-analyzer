...
function buildHealthEntries(healthStatus) {
  if (!healthStatus || healthStatus.error) return []
  const entries = []
  ...
  if (healthStatus.osv_synced_at) {
    ...
    entries.push({ level: 'INFO', message: `OSV last synced ${mins < 1 ? 'just now' : `${mins}m ago`}` })
  }
  if (healthStatus.epss_synced_at) {
    entries.push({ level: 'INFO', message: 'EPSS scores loaded' })
  }
  if (healthStatus.kev_synced_at) {
    entries.push({ level: 'INFO', message: 'CISA KEV list loaded' })
  }
  return entries
}

export default function SystemLogs({ logs = SIMULATED_LOGS, healthStatus, maxHeight = 180 }) {
  const [isExpanded, setIsExpanded] = useState(true)
  const [entries, setEntries] = useState(logs || [])
  const containerRef = useRef(null)
  const healthInjected = useRef(false)

  // Inject real health entries once when healthStatus first resolves
  useEffect(() => {
    if (!healthStatus || healthInjected.current) return
    healthInjected.current = true
    const real = buildHealthEntries(healthStatus)
    if (real.length > 0) setEntries(prev => [...prev, ...real])
  }, [healthStatus])
